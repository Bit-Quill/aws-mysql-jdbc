...
|`gatherPerfMetrics` | Boolean | No | Set to true if you would like the driver to record failover-associated metrics. <br><br>To print collected metrics, call `ClusterAwareMetricReporter.reportMetrics(String connUrl, Log log, boolean atClusterLevel)`. <br><br>To reset collected metrics, call `ClusterAwareMetricReporter.reportMetrics(String connUrl, Log log, boolean atClusterLevel)`. | `false` | 
|`perfMetricAtClusterLevel` | Boolean | No | Set to true if you would like the failover performance metrics tracked per cluster. Set to false to collect failover performance metrics per instance. | `true` | 
